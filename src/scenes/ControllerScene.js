...
export default class ControllerScene extends Phaser.Scene {
  ...
  onRoster(m) {
    ...
    // Match frozen while another seat is held open after an accidental drop. A
    // device that is itself reconnecting already shows its own banner, so don't
    // stack the two (single :after pseudo-element).
    this.paused = !!m.paused;
    if (this.overlay) this.overlay.classList.toggle('paused', this.paused && !this.reconnecting);
  ...
  refresh() {
    ...
    this.overlay.classList.toggle('paused', !!this.paused && !this.reconnecting);
...
function battleLabel(on) {
  return on ? '⚔ Living battlefield' : 'Tower duel';
}
...
function injectControllerStyles() {
  ...
  style.textContent = `
    ...
    .tp-ctl.paused:after{content:'⏸ Paused — waiting for a player…';position:fixed;top:0;left:0;right:0;z-index:30;
      text-align:center;padding:8px;background:#c9892b;color:#fff;font-weight:bold;font-size:16px;}
