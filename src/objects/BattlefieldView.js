...
export default class BattlefieldView {
  ...
  drawIntendant(g, bf) {
    // §4.1bis: in living mode the Intendant only EXISTS once a third player holds
    // the seat (server: setPresent(!!intendant)). Until then there is no avatar to
    // draw — he enters from the sky on connect (the `apparition` event + descent).
    if (!bf.present) { this._prevIx = null; this._deathT0 = null; return; }
    const I = bf.intendant;
    if (I.dead) {
      // Death animation: a local clock started the first frame we see `dead` flip
      // (the sim's `intFatal` event fires the SFX on the same transition, so the
      // visual and the audio stay in lockstep). Reset when he revives (apparition).
      const now = this.scene.time.now;
      if (this._deathT0 == null) this._deathT0 = now;
      this.drawIntendantDeath(g, bf, (now - this._deathT0) / 1000);
      this._prevIx = null;
      return;
    }
    this._deathT0 = null;
  ...
  // The Intendant's death throes, played off a local clock `e` (seconds since the
  // `dead` flag flipped). Four overlapping beats, each gated by `e`:
  //   0.00–0.50  killing burst — a shock ring + radial debris at the buste
  //   0.00–0.80  the avatar topples to the ground (ease-out), then fades by ~1.3s
  //   0.25–1.60  the soul departs — motes rising and fading upward
  //   ≥1.30      a faint residual marker pulses on the spot until he respawns
  // The burst/soul tint mirrors the biome's shield palette (drawShield) so death
  // reads as the same magic failing — vortex ember or égide gold, never a name test.
  drawIntendantDeath(g, bf, e) {
    const I = bf.intendant;
    const x = I.x; const y = I.y;
    const biome = this.scene.biome;
    const style = (biome && biome.intendantShield) || 'egide';
    const burstCol = style === 'vortex' ? ((biome && biome.ambientColor) || 0x96dcff) : 0xf0d29a;
    const t = this.scene.time.now / 1000;

    // 1) KILLING BURST — expanding white shock ring + a coloured inner ring and
    //    radial debris streaks, all fading as they grow (first 0.5s).
    if (e < 0.5) {
      const f = e / 0.5;                    // 0 → 1
      const rr = 6 + f * 34;
      g.lineStyle(3 * (1 - f), 0xfff0c8, 1 - f); g.strokeCircle(x, y - 10, rr);
      g.lineStyle(2 * (1 - f), burstCol, 0.8 * (1 - f)); g.strokeCircle(x, y - 10, rr * 0.6);
      g.lineStyle(1.5 * (1 - f), burstCol, 1 - f);
      for (let k = 0; k < 8; k += 1) {
        const a = k * (Math.PI / 4) + 0.3; const r0 = rr * 0.5; const r1 = rr;
        g.lineBetween(x + Math.cos(a) * r0, y - 10 + Math.sin(a) * r0, x + Math.cos(a) * r1, y - 10 + Math.sin(a) * r1);
      }
    }

    // 2) TOPPLING BODY — the REAL avatar (drawIntBody, same style sent on the
    //    wire), now fading via drawIntBody's alpha multiplier. It topples ~90°
    //    toward his facing, sinks, holds briefly, then dissolves by ~1.3s.
    const fall = Math.min(1, e / 0.8);
    const ease = 1 - (1 - fall) * (1 - fall);
    const lean = ease * (Math.PI / 2) * (I.facing || 1);
    const sink = ease * 4;
    const bodyA = Math.max(0, 1 - Math.max(0, (e - 0.8) / 0.5));
    // The head/eyes lead the dissolve — gone (~1.05s) well before the body (~1.3s).
    const headA = Math.max(0, 1 - Math.max(0, (e - 0.55) / 0.45));
    if (bodyA > 0.01) {
      g.save();
      g.translateCanvas(x, y - sink);
      g.rotateCanvas(lean);
      g.save(); g.scaleCanvas(I.facing || 1, 1);
      this.drawIntBody(g, I.style, 0, false, false, bodyA, headA);
      g.restore();
      g.lineStyle(1.6, burstCol, bodyA * 0.85); g.lineBetween(-4, -12, 4, -6); // killing slash
      g.restore();
    }

    // 3) DEPARTING SOUL — a few motes drifting up and out, fading as they rise.
    if (e > 0.25 && e < 1.6) {
      const rise = e - 0.25;
      for (let k = 0; k < 4; k += 1) {
        const ph = k * 0.618;
        const ry = y - 14 - rise * (26 + k * 6);
        const rx = x + Math.sin((rise + ph) * 3) * (3 + k);
        const a = Math.max(0, 0.7 - rise * 0.5);
        if (a <= 0) continue;
        g.fillStyle(burstCol, a * 0.5); g.fillCircle(rx, ry, 3.4 - k * 0.3);
        g.fillStyle(0xeaf2ff, a); g.fillCircle(rx, ry, 2.2 - k * 0.3);
      }
    }

    // 4) WILL-O'-THE-WISP — once the body is gone the soul lingers as a small
    //    spirit-flame over the spot. It wanders irregularly (summed incommensurate
    //    sines → organic, seedless drift) and shimmers slowly; a dim ground mark
    //    anchors the exact point of the fall. Holds until the next round / respawn.
    //    Tinted by the biome shield palette (burstCol).
    if (e >= 1.3) {
      const appear = Math.min(1, (e - 1.3) / 0.4);                  // fade-in, no pop
      const wx = x + Math.sin(t * 0.9) * 3 + Math.sin(t * 2.3 + 1.7) * 1.5;
      const wy = y - 8 + Math.sin(t * 1.3 + 0.5) * 2.2 + Math.cos(t * 0.7) * 1.1;
      let fl = 0.55 + 0.3 * Math.sin(t * 1.5) + 0.12 * Math.sin(t * 3.7 + 2); // slow shimmer
      fl = Math.max(0.2, Math.min(1, fl)) * appear;
      const r = 3 + 0.7 * Math.sin(t * 1.9 + 1);
      g.fillStyle(0x555a6b, 0.22 * appear); g.fillCircle(x, y - 4, 3);   // ground anchor
      g.fillStyle(burstCol, 0.10 * fl); g.fillCircle(wx, wy, r * 2.6);   // outer halo
      g.fillStyle(burstCol, 0.20 * fl); g.fillCircle(wx, wy, r * 1.5);   // inner halo
      g.fillStyle(burstCol, 0.55 * fl); g.fillCircle(wx, wy, r);         // flame body
      g.fillStyle(0xeaf2ff, 0.9 * fl); g.fillCircle(wx, wy, r * 0.55);   // bright core
    }
  }
  ...
  // `a` is a global alpha multiplier (default 1) so callers can fade the whole
  // avatar — e.g. the death dissolve — without re-deriving every fill colour.
  // `headA` (defaults to `a`) fades the FACE/EYES independently: the death
  // dissolve drops it ahead of `a` so the head (robe) / the eyes (mascot) vanish
  // before the body. Everything else stays on `a`.
  drawIntBody(g, style, t, w, air, a = 1, headA = a) {
    const legA = (amp) => (air ? amp * 0.6 : (w ? Math.sin(t) * amp : 0));
    if (style === 2) {                           // worker (helmet)
      const ls = legA(3); const rs = -ls;
      g.lineStyle(2, 0x7a5b2a, a);
      g.lineBetween(-1, -7, -1 + ls, 0); g.lineBetween(1, -7, 1 + rs, 0);
      g.fillStyle(0xf2c14e, a); g.fillRect(-3, -15, 6, 9);
      const as = air ? -3 : Math.sin(t + Math.PI) * 2.5;
      g.lineStyle(1.5, 0xd9a93f, a); g.lineBetween(2, -13, 4 + as, -8);
      g.fillStyle(0xffe0a0, headA); g.fillCircle(0, -18, 3.5);            // face → headA
      g.fillStyle(0xe6685a, a);                                          // helmet stays on a
      ...
      g.fillStyle(0x7ee0c0, a);
      g.fillTriangle(-1, -15, -7, -9 + flap, -1, -5);
      const ls = legA(4); const rs = -ls;
      g.lineStyle(2, 0xcaa14a, a);
      g.lineBetween(0, -7, ls, 0); g.lineBetween(0, -7, rs, 0);
      g.fillStyle(0xf2c14e, a); g.fillRect(-2.5, -16, 5, 10);
      g.fillStyle(0xffe0a0, headA); g.fillCircle(0, -19, 3.6);           // face → headA
      g.fillStyle(0xcaa14a, a); g.fillTriangle(-2, -20, -5, -22, -2, -17); // hat stays on a
    } else if (style === 4) {                    // mascot (squash)
      const sq = air ? 1.18 : (w ? 1 + Math.sin(t * 2) * 0.09 : 1);
      g.save(); g.scaleCanvas(1 / sq, sq);
      g.fillStyle(0xf2c14e, a); g.fillCircle(0, -9, 7.5);                // ball stays on a
      g.restore();
      g.fillStyle(0xffffff, headA); g.fillCircle(2, -12, 1.8); g.fillCircle(-1.5, -12, 1.8); // eyes → headA
      g.fillStyle(0x1a1300, headA); g.fillCircle(2.4, -12, 0.9); g.fillCircle(-1.1, -12, 0.9);
      const ls = legA(2);
      g.lineStyle(2.5, 0xcaa14a, a);
      ...
      g.fillStyle(0xa87c28, a);
      g.fillTriangle(-6, 0, 6, 0, 6 + sway, 3);
      g.fillTriangle(-6, 0, 6 + sway, 3, -6 + sway, 3);   // animated hem
      g.fillStyle(0xf2c14e, headA); g.fillCircle(0, -18, 4.3);           // head → headA (goes first)
      g.fillStyle(0xa87c28, a);                                          // hood stays on a
      g.beginPath(); g.arc(0, -19, 5.4, Math.PI * 1.02, Math.PI * 1.98); g.fillPath();
      g.restore();
    }
  }
  ...
  drawShield(g, bf) {
    const I = bf.intendant;
    const sh = I && I.shield;
    if (!bf.present || !I || I.dead || !sh || sh.t <= 0) return;
    const R = SHIELD_R;
