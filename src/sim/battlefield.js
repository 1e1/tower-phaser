...
export const DEFAULT_PARAMS = {
  ...
  grenWind: 0.6, grenRec: 0.15,                // grenadier: arm → throw → after-burst
  ...
  engHp: 3, engBridgeMax: 6, engLadH: 40, engLadRun: 52,
...
export default class Battlefield {
  ...
  // }
  constructor({ seed = 12345, world, params = {} } = {}) {
    this.seed = (seed >>> 0) || 1;
    ...
    this.I = {
      ...
      style: 1, step: 0, walking: false, jumpY: -9999, bp: [0, 0],
  ...
  newSoldier(owner) {
    ...
    const s = { id: this.nextId++, owner, x: xd, y: this.platLine[owner], dir, kind, hp, ft: 0, stuck: 0, deserter: false, st: 'emerge', t: this.C.doorT, age: 0, fall: false, jseed, repTime };
    if (plan) s.plan = plan;
    return s;
  }
  ...
  planEngineer(owner) {
    const p = this.buildPlan(owner); if (!p) return null; const dir = p.dir; const base = this.navReach(owner);
    let rT;
  ...
  step(dt) {
    ...
    if (this.ruinT > 0) {
      this.ruinT -= dt;
      if (this.ruinT <= 0) {
        if (this.loserOwner >= 0) { if (!this.roundScored) this.showMsg('✗ Intendant fails — no soldier reached the ruin'); this.nextRound(); }
        else { this.horde = []; this.ruinSide = -1; }
      }
    }
    ...
    if (this.loserOwner >= 0 && !this.roundScored && this.horde.some((h) => h.st === 'arrived')) {
      this.roundScored = true; this.score += 1; this.showMsg('✓ Intendant succeeds — a soldier reached the ruin');
    }
    if (this.ruinT > 0 && this.horde.length > 0 && this.horde.every((h) => h.st === 'arrived' || h.hp <= 0)) this.ruinT = Math.min(this.ruinT, 1.0);
    ...
    for (const s of arr) {
      ...
      if (interact && !pushing) {
        ...
        if (s.kind === 'cata') {
          ...
          let tx = null; let ty = 0; let bd = C.arRange;
          ...
          let tx = null; let ty = 0; let bd = C.musRange;
      ...
      const onLad = (n, lx, ly) => Math.abs(n.x - lx) <= 8 && Math.abs(n.y - ly) < 2;
  ...
  snapshot() {
    return {
      ...
      horde: this.horde.map((s) => ({ id: s.id, owner: s.owner, x: Math.round(s.x), y: Math.round(s.y), st: s.st, bg: !!s.bg, dir: s.dir })),
