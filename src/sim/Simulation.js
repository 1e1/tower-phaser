...
export default class Simulation {
  ...
  battlefieldWorld() {
    ...
    return {
      ...
      dig(tx, ty, rr, amount, digH = 9) {
        // Pit floor: bottoms out at the stable rim (min of the two shoulders
        // OUTSIDE the hole) + a bounded depth, so it never digs to infinity. But
        // each column's target is taken below max(rim, current surface) — the old
        // `rim + digH` left dead zones wherever the rim sat BELOW the spot under
        // the tool (a hilltop, a downhill, a natural dip): fy fell at/under the
        // surface and nothing moved. Referencing the higher of rim/surface
        // guarantees a real bite anywhere while the rim still bounds the bottom.
        const rim = Math.min(self.heights[self.clampX(tx - rr - 6)], self.heights[self.clampX(tx + rr + 6)]);
        const bottom = rim + Math.max(40, digH * 5);
        for (let d = -rr; d <= rr; d += 1) { const w = 0.5 + 0.5 * Math.cos((Math.PI * d) / rr); const c = self.clampX(tx + d); const fy = Math.min(bottom, Math.max(rim, self.heights[c]) + w * digH); if (self.heights[c] < fy) self.heights[c] = Math.min(GAME_HEIGHT - 2, fy, self.heights[c] + amount * w); }
      },
  ...
  buildBattlefield() {
    ...
    this.battlefield = new Battlefield({ seed: this.seed ^ 0x9e3779b9, world: this.battlefieldWorld(), params: { maxHp: this.maxHp }, intendantBody: this.biome.intendantBody });
    // Real artillery drives the escalade cannon's cadence (see onArtilleryFired).
    this.battlefield.syncCannon = true;
    // This Simulation rebuilds a fresh battlefield each round, so the living sim
    // must NOT self-advance on its end verdict — it only signals (endDone) and we
    // gate applyResolution on it (RESOLVING below).
    this.battlefield.externalRounds = true;
    // Carry the previous round's winner forward as the Intendant's aggressor: the
    // camp that won becomes the side the Intendant fights (and that fights him),
    // while blue and red keep warring with each other. Round 1 (no prior tower
    // fell) leaves him neutral. A FRESH Battlefield is built every round, so the
    // alignment can't survive in the discarded instance — it must be re-applied
    // here, or round 2+ would start with a neutral, unattacked Intendant.
    if (this.lastDestroyed != null) this.battlefield.setInvader(1 - this.lastDestroyed);
  }
  ...
  tick(dt) {
    ...
    if (this.phase === PHASE.FIRING) {
      ...
      if (this.resolveTimer <= 0) {
        // Living mode: a tower fell → hold the arena pan until the winner's horde
        // has actually rendered its verdict on screen (reached the ruin, was wiped
        // out, or surrendered and fell back). The battlefield's ruinCap backstops
        // this, so a stuck horde can never hang the transition.
        if (this.battlefield && this.battlefield.endSeq && !this.battlefield.endDone) return;
