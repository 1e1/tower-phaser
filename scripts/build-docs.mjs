...
// ── annex listing (single source of truth: site.config `pages`) ──────────────
// Each annex declares its own card (card.lot/title/desc) in its catalog. The
// hub grid and the home teaser are GENERATED by walking the annexes in config
// order, so adding an annex to `pages` lists it in both places automatically —
// no more hand-kept lists drifting out of sync. The card biome reuses the
// annex's own biome, so its hero hills and its card tint match.
const annexPages = pages.filter((p) => p.kind === 'annex' && p.slug !== 'annex');

function annexCard(page, lang) {
  const cat = readJSON(join(i18nDir, `${page.slug}.${lang}.json`)) || {};
  const get = (k) => {
    if (cat[k] != null) return cat[k];
    problems.push(`card: ${page.slug}.${lang} missing ${k}`);
    return `{{${k}@${page.slug}}}`; // visible in output AND listed as an issue
  };
  return { slug: page.slug, biome: page.biome || 'meadow', lot: get('card.lot'), title: get('card.title'), desc: get('card.desc') };
}

// Hub grid card, one per annex (biome tint + lot/title/desc).
function annexGrid(lang) {
  return annexPages.map((p) => {
    const c = annexCard(p, lang);
    return `          <a class="chap-card ${c.biome}" href="${c.slug}.html">\n            <div class="lot">${c.lot}</div>\n            <h3>${c.title}</h3>\n            <p>${c.desc}</p>\n          </a>`;
  }).join('\n');
}

// Home teaser <li>, one per annex. The leading emoji is the first token of the
// card's `lot` (e.g. "🧮 Maths" → "🧮"), so the home keeps its icons for free.
function annexList(lang) {
  return annexPages.map((p) => {
    const c = annexCard(p, lang);
    const emoji = String(c.lot).split(' ')[0];
    return `          <li>${emoji} <a href="${c.slug}.html"><b>${c.title}</b></a> — ${c.desc}</li>`;
  }).join('\n');
}
...
for (const page of pages) {
  ...
  for (const lang of langs) {
    ...
    // Generated annex listings (only the hub and the home reference them).
    if (page.slug === 'annex') cat.annexGrid = annexGrid(lang);
    if (page.kind === 'home') cat.annexList = annexList(lang);
